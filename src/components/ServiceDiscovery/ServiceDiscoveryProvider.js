...
const ServiceDiscoveryProvider = ({ children }) => {
  ...
  const setIsLoading = (isLoading) =>
    dispatch({ type: Actions.SetVarsLoading, payload: isLoading });
  const setIsError = (error) =>
    dispatch({ type: Actions.SetVarsError, payload: error });

  // load vars.json for service discovery
  useEffect(() => {
    (async () =>
      configureAuth(
        await loadVarsJson({ store, dispatch, intl, setIsLoading, setIsError })
      ))();
  }, []);
      ...
      <FullScreenLayout>
        <Alert
          message={intl.formatMessage(
            Messages.Text_System_ErrorMessage_ServiceDiscovery
          )}
          type="error"
          showIcon
        />
